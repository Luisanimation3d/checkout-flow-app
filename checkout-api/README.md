...
- **HTTPS en toda la cadena pública**: tanto el frontend (CloudFront) como la API (segunda distribución CloudFront delante de Elastic Beanstalk) sirven exclusivamente sobre HTTPS.
- **Headers de seguridad (Helmet)**: `main.ts` aplica [`helmet`](https://helmetjs.github.io/) globalmente — `Strict-Transport-Security`, `X-Content-Type-Options`, `X-Frame-Options`, `X-DNS-Prefetch-Control`, `X-Download-Options`, `X-Permitted-Cross-Domain-Policies`, etc. Se desactiva únicamente la Content-Security-Policy por defecto, porque bloquea los assets inline que carga Swagger UI (`/api-docs`); el resto de headers queda activo para toda la API.
